...
${birthData.planetDetails?.filter((p) => p.id === "rahu" || p.id === "ketu" || p.name.includes("\u4EA4\u9EDE")).map((p) => `  * ${p.name}: ${p.signName} ${Math.floor(p.degreeInSign)}\u5EA6, \u843D\u5165\u7B2C ${p.house} \u5BAE (${p.houseMeaning})`).join("\n") || ""}
